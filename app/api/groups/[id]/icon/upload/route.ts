  ...
  try {
    ...
    if (group.image) {
      const isUploadedImage = 
        group.image.startsWith('/uploads/group-pictures/') ||
        (group.image.includes('blob.vercel-storage.com') && group.image.includes('group-pictures'))
    ...
    const uniqueFileName = `${groupId}/${timestamp}-${randomString}${fileExtension}`

    // Upload file (uses local storage in dev, Vercel Blob in production)
    const result = await uploadFile(uniqueFileName, file, file.type, 'group-pictures')

    // Update group's image URL in database
    ...
    if (error instanceof Error) {
      ...
      return NextResponse.json(
        { error: error.message || 'Failed to upload group picture' },
        { status: 500 }
      )
    }
